...
import {
	...
	LOOP_DIR_NAME,
	PROJECT_LOOP_DIR_NAME,
	PROJECTS_DIR_NAME,
} from "shared/constants";
import type { LocalSetupConfig, SetupConfig } from "shared/types";

/**
 * Worktrees don't include gitignored files, so copy .loop from main repo
 * if it's missing — ensures setup scripts like "./.loop/setup.sh" work.
 */
export function copySupersetConfigToWorktree(
	mainRepoPath: string,
	worktreePath: string,
): void {
	const mainSupersetDir = join(mainRepoPath, PROJECT_LOOP_DIR_NAME);
	const worktreeSupersetDir = join(worktreePath, PROJECT_LOOP_DIR_NAME);
	...
	if (existsSync(mainSupersetDir) && !existsSync(worktreeSupersetDir)) {
		try {
			...
			console.error(
				`Failed to copy ${PROJECT_LOOP_DIR_NAME} to worktree: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
}
...
function readConfigFromPath(basePath: string): SetupConfig | null {
	return readConfigFile(
		join(basePath, PROJECT_LOOP_DIR_NAME, CONFIG_FILE_NAME),
	);
}
...
function readLocalConfigFromPath(basePath: string): LocalSetupConfig | null {
	return readLocalConfigFile(
		join(basePath, PROJECT_LOOP_DIR_NAME, LOCAL_CONFIG_FILE_NAME),
	);
}
 ...
 *   1. User override:  ~/.loop/projects/<projectId>/config.json
 *   2. Worktree:       <worktreePath>/.loop/config.json
 *   3. Main repo:      <mainRepoPath>/.loop/config.json
 *
 ...
 * `.loop/config.local.json` exists in the workspace (worktree or main repo).
