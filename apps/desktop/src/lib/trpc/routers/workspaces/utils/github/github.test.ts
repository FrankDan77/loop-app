...
describe("getPullRequestRepoArgs", () => {
	test("returns upstream repo args for forks", () => {
		expect(
			getPullRequestRepoArgs({
				isFork: true,
				upstreamUrl: "git@github.com:FrankDan77/loop-app.git",
			}),
		).toEqual(["--repo", "FrankDan77/loop-app"]);
	});
...
describe("shouldRefreshCachedRepoContext", () => {
	test("returns false when no cached repo context exists", () => {
		expect(
			shouldRefreshCachedRepoContext({
				originUrl: "https://github.com/FrankDan77/loop-app",
						...
						nodes: [
							{
								...
								url: "https://github.com/FrankDan77/loop-app/pull/2681#discussion_r42",
			...
			parseConversationCommentsResponse([
				{
					...
					html_url:
						"https://github.com/FrankDan77/loop-app/pull/2681#issuecomment-7",
				},
			]),
		).toEqual([
			{
				...
				url: "https://github.com/FrankDan77/loop-app/pull/2681#issuecomment-7",
				kind: "conversation",
				isResolved: false,
			},
		]);
