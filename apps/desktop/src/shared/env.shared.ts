...
const envSchema = z.object({
	...
	LOOP_WORKSPACE_NAME: z.string().default("loop"),
});
...
export const env = envSchema.parse({
	...
	LOOP_WORKSPACE_NAME: process.env.LOOP_WORKSPACE_NAME,
});

export function getWorkspaceName(): string | undefined {
	const name = env.LOOP_WORKSPACE_NAME;
	if (name === "loop") return undefined;
	return name
		.toLowerCase()
		.replace(/[^a-z0-9-]/g, "-")
		.slice(0, 32);
