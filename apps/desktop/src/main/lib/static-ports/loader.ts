...
import { PORTS_FILE_NAME, PROJECT_LOOP_DIR_NAME } from "shared/constants";
import type { StaticPortsResult } from "shared/types";

/**
 * Load and validate static ports configuration from a worktree's .loop/ports.json file.
 *
...
export function loadStaticPorts(worktreePath: string): StaticPortsResult {
	const portsPath = join(worktreePath, PROJECT_LOOP_DIR_NAME, PORTS_FILE_NAME);
 ...
 *
 * @param worktreePath - Path to the workspace's worktree directory
 * @returns true if .loop/ports.json exists
 */
