...
const SPAWN_LOCK_PATH = join(LOOP_HOME_DIR, "terminal-host.spawn.lock");
const SCRIPT_MTIME_PATH = join(LOOP_HOME_DIR, "terminal-host.mtime");
...
export class TerminalHostClient extends EventEmitter {
	...
	private async spawnDaemon(): Promise<void> {
		...
		try {
			...
			const logPath = join(LOOP_HOME_DIR, "daemon.log");
			let logFd: number;
			try {
