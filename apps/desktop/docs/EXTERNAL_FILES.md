...
`~/.loop[-{workspace}]/bin/codex` still injects `notify` and keeps the
...
|-------|---------|--------------|
| Zsh | `~/.zshrc` | Prepends `~/.loop[-{workspace}]/bin` to PATH |
| Bash | `~/.bashrc` | Prepends `~/.loop[-{workspace}]/bin` to PATH |
...
| `LOOP_WORKSPACE_NAME` | Human-readable workspace name |
| `LOOP_WORKSPACE_PATH` | Filesystem path to the workspace |
| `LOOP_ROOT_PATH` | Root path of the project |
...
Before adding new files outside of `~/.loop[-{workspace}]/`:
