...
Lives at `~/.loop/host/<organizationId>/manifest.json`. The coordinator writes and reads it. Remote deployments don't use manifests.
...
| Default `~/.loop/host.db` | `app.ts` | Injected by caller | `dbPath` is required, no default |
| `~/.loop/chat-anthropic-env.json` | `anthropic-runtime-env.ts` | Moves with `LocalModelProvider` | Desktop-only path |
...
No `if (process.resourcesPath)`. No `if (platform() === "darwin")`. No `~/.loop` defaults. The host service is a pure server; the caller decides how it's configured.
