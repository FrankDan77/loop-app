...
- [ ] **A2. Prompt + text attachment** — drag `v2-launch-test-artifacts/trace.log`
      into the modal. After launch: verify `.loop/attachments/trace.log`
      exists in the worktree (terminal: `ls .loop/attachments/`).
      Claude prompt contains `![trace.log](.loop/attachments/trace.log)`.
