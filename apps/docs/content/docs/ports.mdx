...
Create `.loop/ports.json` in your repository:
...
**Tips:**
- Commit `.loop/ports.json` to share port labels with your team
- **Pro tip:** If you want deterministic per-workspace port ranges, implement it in setup/teardown scripts by reserving a range in a shared file (for example `~/.loop/port-allocations.json`) during setup and releasing it during teardown. See this repo's examples: [`.loop/setup.sh`](https://github.com/superset-sh/superset/blob/main/.loop/setup.sh) and [`.loop/teardown.sh`](https://github.com/superset-sh/superset/blob/main/.loop/teardown.sh).
