...
|----------|-------------|
| `LOOP_ROOT_PATH` | Path to root repository |
...
```
~/.loop/projects/<repo-path>/config.json
```

For a repo at `/Users/me/work/app`, that's `~/.loop/projects/Users/me/work/app/config.json`. The legacy location keyed by project ID (`~/.loop/projects/<project-id>/config.json`) still works as a fallback.

### Priority Order

For each key (`setup`, `teardown`, `run`), the highest-priority source that defines it wins:

1. `~/.loop/projects/<repo-path>/config.json`: user override
2. `<worktree>/.loop/config.json`: workspace-specific (setup and teardown only)
3. `<repo>/.loop/config.json`: project default
...
If no commands are configured for a key, Superset looks for a script instead: `.loop/<key>.sh` (`setup.sh`, `teardown.sh`, `run.sh`) in the worktree first, then the main repo. A gitignored script isn't checked out into new worktrees, so it typically exists only in the main repo.
...
```json
{
  "setup": ["~/.loop/projects/<repo-path>/setup.sh"],
  "teardown": ["~/.loop/projects/<repo-path>/teardown.sh"]
}
```
...
Extend or override your team's committed scripts without modifying the repo. Create `.loop/config.local.json` alongside `config.json`. It's gitignored automatically.
...
```json
{
  "setup": {
    ...
    "after": ["./.loop/my-post-setup.sh"]
  },
  "teardown": {
    "after": ["./.loop/my-cleanup.sh"]
  }
}
```
...
- Commit `.loop/` to share with team
- Use shell scripts for complex logic: `"setup": ["./.loop/setup.sh"]`
- Use `config.local.json` to add personal steps without touching the team's config
- Use user overrides (`~/.loop/projects/`) to replace scripts entirely per-project
