...
<Command
	...
	options={[
		...
		{
			flag: "--api-key <key>",
			description: "Store a Superset API key (`sk_live_…`) at `~/.loop/config.json` instead of running the OAuth flow.",
		},
	]}
>
...
the key, stores it at `~/.loop/config.json`, and clears any prior
OAuth session.
...
>
Set the active organization in `~/.loop/config.json`.
