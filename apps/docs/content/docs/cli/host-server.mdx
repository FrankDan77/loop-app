...
`~/.loop/bin/superset` shim on app launch. Superset terminals prepend
...
`~/.loop/host/<organizationId>/manifest.json` so other clients can find
...
State lives under `~/.loop/host/<organizationId>/`:

```text
~/.loop/host/<organizationId>/manifest.json    # endpoint + auth token
~/.loop/host/<organizationId>/host.db          # local host server DB
```

A host server started by the desktop app is visible to the CLI and
vice-versa. `LOOP_HOME_DIR` relocates the whole tree.
