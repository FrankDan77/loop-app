...
an app-managed shim at `~/.loop/bin/superset`, and Superset terminals
prepend `~/.loop/bin` to `PATH` automatically.

To use the bundled CLI from a normal shell, add the directory to your shell
profile:

```bash
export PATH="$HOME/.loop/bin:$PATH"
```
...
consent page, and stores a session token at `~/.loop/config.json`.
...
```text
~/.loop/config.json                            # auth, active organization
~/.loop/host/<organizationId>/manifest.json    # host server endpoint + token
~/.loop/host/<organizationId>/host.db          # local host server DB
```

`LOOP_HOME_DIR` relocates the whole tree, matching the desktop app.
