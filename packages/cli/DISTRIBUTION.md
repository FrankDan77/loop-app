...
```
superset start
  └─ reads ~/.loop/config.json (auth token, org ID, API URL)
...
superset start --daemon
  └─ same but detached, writes manifest to ~/.loop/host/<orgId>/manifest.json
...
Detects platform/arch, downloads tarball from GitHub Releases, extracts to `~/.loop/bin/`, prints PATH instructions.
...
# Extract
mkdir -p ~/.loop/bin
tar -xzf superset-darwin-arm64.tar.gz -C ~/.loop/bin

# Add to PATH
export PATH="$HOME/.loop/bin/bin:$PATH"
...
| `packages/cli/src/lib/config.ts` | ~/.loop/config.json read/write |
