...
Two workspace → PR match sites keyed on branch name alone. Any cross-fork PR whose `headRefName` collided with a local branch got attached — e.g. PR #3261 (`quueli/superset-windows:main` → `FrankDan77/loop-app:main`) showing on every local `main` workspace.
...
- PR #3261 (cross-fork): workspace upstream `FrankDan77/loop-app#main` vs PR key `quueli/superset-windows#main` — no match in v2. v1 predicate returns false via `isCrossRepository` check. ✓
